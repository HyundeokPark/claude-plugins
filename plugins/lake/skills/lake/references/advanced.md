...
## 사람용 요약 (📍) — 만들지 않고 수확한다 (v1.10.0)

### 왜 수확인가

resume 출력이 AI용이라 사람이 본질을 파악하기 어렵다는 지적에서 출발했다. 그런데
**Claude Code가 이미 같은 것을 쓰고 있었다** — 사용자가 자리를 비웠다 돌아오면 대화 전체를
보고 요약을 만들어 트랜스크립트에 `type=system, subtype=away_summary`로 남긴다.

```
NestAds 컨텍스트 카드 고도화 실험을 마쳤고, 계획과 구현 A-B 모두 무승부라는 결론까지
lake(e8b930)에 저장해 뒀습니다. 다음은 카드 v2 교체 여부를 결정해 주시는 것입니다.
```

실측 5건이 전부 108~176자에 "무엇을 하던 중 → 어디까지 됐다 → 다음은 이것" 3박자였다.
이 품질을 프롬프트로 재현할 수 없는 이유는 분명하다 — **lake의 spool에는 도구 호출과
사용자 프롬프트만 있고 어시스턴트의 판단이 없다.** away_summary는 대화 전체를 봤다.
입력이 다르므로 생성 대신 수확한다. 추가 LLM 호출은 0이다.

### 2단 경로

```
세션 종료 (SessionEnd → lake-compactor.js)
  │
  ├─ 1순위  트랜스크립트에서 away_summary 수확
  │          경로 탐색: ~/.claude/projects/*/{session_id}.jsonl
  │            (cwd→디렉터리명 규칙에 의존하지 않고 스캔 — session_id가 UUID라 안전)
  │          구간(segmentByTask)의 시간 창에 든 것만, 여러 개면 마지막
  │          꼬리의 "(disable recaps in /config)" 안내문 제거
  │          커버리지 약 53% — 자리를 비운 세션에만 생긴다
  │
  └─ 2순위  compactor의 기존 haiku 호출에 붙인 ===RECAP=== 블록
             (추가 호출이 아니다. 같은 호출에 블록 하나 더)
  │
  ▼
spec.md 맨 위 `## 📍 사람용 요약`  (제목 바로 아래, 메타데이터 불릿보다 위)
  │
  ├─→ brief 최상단 (📌 Goal 위 — 사람이 제일 먼저 읽는 자리)
  ├─→ recap view: Status 다음 `📍 …` 한 줄
  └─→ SessionStart 브리핑: auto-context보다 우선 사용
```

### 수동이 자동을 이긴다

자동 생성분에만 `<!-- lake:auto-recap -->` 마커를 달고, **마커가 있을 때만 덮어쓴다.**
마커를 지우고 직접 쓰면 그 뒤로 자동은 손대지 않는다(`manual-kept`).

context.md처럼 mtime을 비교하지 않는 이유: spec.md는 resume·Stop 훅이 Updated
타임스탬프를 계속 갱신하므로 mtime으로는 사람의 편집을 구분할 수 없다.

### 시간 창을 보는 이유

한 세션이 여러 태스크를 오갈 수 있다(구간 분리는 v1.8.0). 창을 안 보고 마지막 요약을
쓰면 **다른 태스크의 상태를 이 태스크에 적는다.** 창 안에 없으면 수확하지 않는다 —
틀린 요약보다 없는 게 낫다.

### 곁들여 고친 것

`extractSpecGoal`이 `## Goal`만 찾아서, 한글 `## 목표`를 쓴 spec은 폴백으로 **앞 22줄이
통째로** 출력됐다. 이게 "정보 과다"의 직접 원인이었다. `## 목표`를 같이 인식하고
폴백 분량도 10줄로 줄였다.
